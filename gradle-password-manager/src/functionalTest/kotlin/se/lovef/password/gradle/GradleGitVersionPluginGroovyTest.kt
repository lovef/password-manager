...
import se.lovef.test.properties
...
fun File.readSalt(passwordIdentifier: String) =
    properties("$passwordIdentifier.properties").let { it["salt"] }

fun File.passwordManager(passwords: Passwords) = PasswordManagerImpl(
    SaltFileFactoryImpl(this),
    PasswordHasher(),
    ReadableFormatter(),
    passwords)

class Passwords(vararg passwords: Pair<String, String>) : PasswordReader {
    private val passwords = passwords.toMap()
    override fun get(passwordIdentifier: String) = passwords.getValue(passwordIdentifier)
}
